...
class MyUNetFlow(nn.Module):
	'''
	This Network is designed for cycle_gan10
	Feature from Generator is 1*256*64*64
	However, I need 1*2*256*256 as my output result, so I add another 2 transpose convolution
	'''
	def __init__(self, input_channels=2, output_channels=2, down_scales=5, output_scale=2, num_filters_base=64, max_filters=512,
	             downsampling='conv', sigmoid=False, cuda_id=0, filter_ch='2x'):
		super(MyUNetFlow, self).__init__()
		...
		# 								 bias=True),
		# 			  nn.BatchNorm2d(int(ngf * mult1 / 2)),
		# 			  nn.ReLU(True)]
		model += [nn.ConvTranspose2d(512, 128,
										 kernel_size=3, stride=2,
										 padding=1, output_padding=1,
										 bias=True),
					  nn.BatchNorm2d(128),
		...
		model += [nn.ConvTranspose2d(128, 64,
				  ...
				  nn.BatchNorm2d(64),
				  nn.ReLU(True)]
		model += [nn.Conv2d(64, 2, kernel_size=3, stride=1, padding=1, bias=True)]
		model += [nn.Sigmoid()]
		self.conv_output = nn.Sequential(*model)
		# if sigmoid == True:
		# 	self.conv_output = nn.Sequential(
		# 		nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True),
		# 		nn.Sigmoid()
...
class MyUNetFlow2(nn.Module):
	...
	def __init__(self, input_channels=2, output_channels=2, down_scales=5, output_scale=2, num_filters_base=64, max_filters=512,
	             downsampling='conv', sigmoid=False, cuda_id=0, filter_ch='2x'):
		super(MyUNetFlow2, self).__init__()
		...
		model += [nn.ConvTranspose2d(256, 128,
				  ...
				  nn.BatchNorm2d(128),
				  nn.ReLU(True)]
		self.conv_input = nn.Sequential(*model)
