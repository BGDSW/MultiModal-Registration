...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'my_resnet_9blocks':
        net = MyResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'my_resnet_9blocks2':
        net = MyResnetGenerator2(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'my_resnet_9blocks3':
        net = MyResnetGenerator3(1, 1, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    else:
...
class MyResnetGenerator(nn.Module):
    '''
    ################################################
    This generator have the same structure with ResnetGenerator, but I separate encoder and decoder
    '''
...
class MyResnetGenerator2(nn.Module):
    "downsampling 3 times to get smaller feature 256*50*50"
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        """
        assert(n_blocks >= 0)
        super(MyResnetGenerator2, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if(mult>2):
                mult = 4
                model += [nn.Conv2d(ngf * mult, ngf * mult, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
        ...
        for i in range(difference):
            model += [nn.ConvTranspose2d(int(ngf * mult / 2), int(ngf * mult / 2),
                                         kernel_size=3, stride=2,
                                         padding=1, output_padding=1,
                                         bias=use_bias),
                      norm_layer(int(ngf * mult / 2)),
...
class MyResnetGenerator3(nn.Module):
    "This is designed for cycle_gan7, generate fake image and vessel multi task"
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        """
        assert(n_blocks >= 0)
        super(MyResnetGenerator3, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        mult1 = mult
        for i in range(n_downsampling):  # add upsampling layers
            mult1 = 2 ** (n_downsampling - i)
            model += [nn.ConvTranspose2d(ngf * mult1, int(ngf * mult1 / 2),
                                         kernel_size=3, stride=2,
                                         padding=1, output_padding=1,
                                         bias=use_bias),
                      norm_layer(int(ngf * mult1 / 2)),
                      nn.ReLU(True)]
        model += [nn.ReflectionPad2d(3)]
        model += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model += [nn.Tanh()]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                      ...
                      nn.ReLU(True)]
        model += [nn.ReflectionPad2d(3)]
        model += [nn.Conv2d(ngf, 2, kernel_size=7, padding=0)]
        model += [nn.Tanh()]

        # self.model = nn.Sequential(*model)
        self.Decoder2 = nn.Sequential(*model)

    def forward(self, input, require_feature=False):
        """Standard forward"""
        feature = self.Encoder(input)
        fake = self.Decoder1(feature)
        vessel = self.Decoder2(feature)
        if require_feature:
            return fake, vessel, feature
        else:
            return fake, vessel
...
class AffineNetwork(nn.Module):
    ...
    def __init__(self, in_channels_a, in_channels_b, height, width):
        """
        ...
        n_downsampling = 5
        mult = in_channels_a + in_channels_b
        for i in range(n_downsampling):  # add downsampling layers
        ...
        model += [nn.Conv2d(mult, mult // 2, kernel_size=3, stride=2, padding=1),
                      nn.BatchNorm2d(mult // 2),
                      nn.ReLU(True)]
        # now 8 * 25 * 25
        self.convs = nn.Sequential(*model)
        self.local = nn.Sequential(
            nn.Linear(mult*height*width//8, 1024, bias=True),
            nn.ReLU(True),
...
class AffineSTN(nn.Module):
    ...
    def __init__(self, nc_a, nc_b, height, width):
        super(AffineSTN, self).__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.net = AffineNetwork(nc_a, nc_b, height, width)
