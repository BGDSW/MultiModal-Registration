...
class UnetSTN(nn.Module):
    ...
    def __init__(self, in_channels_a, in_channels_b, height, width, cfg='A', init_func='normal', stn_bilateral_alpha=0,
                 init_to_identity=False, multi_resolution_regularization=1, padding='zeros'):
    ...
    def forward(self, img_a, img_b, apply_on=None, require_field=False):
        """
        ...
        warped_images = []
        for img in apply_on:
            warped_images.append(F.grid_sample(img, resampling_grid, mode=sampling_mode, padding_mode=self.padding_mode,
                                               align_corners=sampling_align_corners))
...
class UPUnetSTN(nn.Module):
    """This class is generates and applies the deformable transformation on the input images."""

    def __init__(self, in_channels_a, in_channels_b, height, width, cfg='A', init_func='normal', stn_bilateral_alpha=0,
                 init_to_identity=False, multi_resolution_regularization=1):
        super(UPUnetSTN, self).__init__()
        self.oh, self.ow = height, width
        self.in_channels_a = in_channels_a
        self.in_channels_b = in_channels_b
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
